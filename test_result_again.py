...
def autoregressive_generation(model, tokenizer, initial_input_str, num_steps, generate_length, temp):
    ...
    for step in range(num_steps):
        ...
        print("Step ", step, " generated: ", tokenizer.sequences_to_texts(sample[:1])[0])
        output_str = extract_sample(tokenizer.sequences_to_texts(sample[:1])[0])
        print("Output string: ", output_str)
        generated_states.append(output_str)
        current_input = f"@PredictNextNextState<{output_str}>"
...
def save_data_to_csv(df, row_indices, autoreg_list, temp, base_dir="AR_result"):
    ...
    for i, row_index in enumerate(row_indices):
        ...
        gt_row = [initial_state] + gt_states[2:2*num_steps+1:2]
...
def create_gifs_from_csv(csv_path_pattern, temp, output_dir="Figures", fps=5):
    ...
    for i in range(num_samples):
        frames = []
        for j in range(1, num_columns):  # For each state (starting from State 2)
...
if __name__ == "__main__":
    ...
    def generate_data(df, future_steps):
        ...
        for i in range(len(df['State 1'])):
            ...
            if future_state_col in df.columns:
                s = f"{start_char}PredictNextNextState<{df['State 1'][i]}> [{df[future_state_col][i]}]{end_char}"
                X_data.append(s)
        return X_data
    ...
    model_path = f"model_parameters/07_22_2024_Conway_2_State_Jump_Rot_Pos_On_Masking_On_Broad_Entropy_Homog_2025-04-12 08-39-12/LifeGPT_v7_epoch_30.pt"
    ...
    temperatures = [0]
    num_steps = 15  # Number of steps to generate (excluding the initial state)
    ...
    for temp in temperatures:
        ...
        row_indices = []
        selected_indices = list(range(7, 8))
